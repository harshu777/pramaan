...
# Production Deployment Script (SQLite - No Docker)
#############################################
...
print_banner() {
    echo -e "${GREEN}"
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║   Certificate Management System - Production Deploy       ║"
    echo "║   SQLite + IPFS + Blockchain (No Docker)                  ║"
    echo "╚════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
setup_environment() {
    ...
    if [ ! -f backend/.env ]; then
        ...
        # Update .env for SQLite (no PostgreSQL)
...
start_blockchain() {
    ...
    if lsof -Pi :8545 -sTCP:LISTEN -t >/dev/null 2>&1; then
        log_warning "Port 8545 is already in use. Blockchain already running..."
        return
    fi
...
print_summary() {
    ...
    echo "  🌐 Admin Login:        http://localhost:3000/static/login.html"
    echo "  🌐 Athlete Login:      http://localhost:3000/static/athlete-login.html"
    echo "  🌐 Admin Dashboard:    http://localhost:3000/static/dashboard.html"
    echo "  🌐 Athlete Dashboard:  http://localhost:3000/static/athlete-dashboard.html"
    ...
    echo "  💾 Database:           SQLite (backend/certificates.db)"
    ...
    echo "  🛑 Stop services:      ./deploy-sqlite.sh stop"
    echo "  🔄 Restart services:   ./deploy-sqlite.sh restart"
    echo "  🧹 Clean up:           ./deploy-sqlite.sh clean"
    echo ""
}
...
# Clean up everything
clean_all() {
    log_warning "This will remove all data including SQLite database!"
